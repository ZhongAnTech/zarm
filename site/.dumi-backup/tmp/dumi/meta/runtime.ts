...
import deepmerge from '/Users/dominicleo/za/zarm/node_modules/.pnpm/deepmerge@4.3.1/node_modules/deepmerge';
export const patchRoutes = ({ routes }) => {
  Object.values(routes).forEach((route) => {
    if (filesMeta[route.id]) {
      if (
        process.env.NODE_ENV === 'production' &&
        (route.meta?.frontmatter?.debug || filesMeta[route.id].frontmatter.debug)
      ) {
