...
export default memo((
  ...
  const columnDataList = useMemo(() => {
    ...
    while (_dataSource) {
      ...
      const childrenData: typeof dataSource | undefined = ((colVal
          ...
          <div className={`${prefixCls}__wrapper`}>
            ...
            <div className={`${prefixCls}__crumbs`}>
...

              {
                errorMsg
                && (
                  <p className={`${prefixCls}__crumbs-error`}>{errorMsg}</p>
                )
              }
            </div>
            ...
            <div className={`${prefixCls}__group`}>
              {
                columnDataList.map((group, index) => {
                  const columnClass = classNames(
                    `${prefixCls}__stack-column`,
                    'za-stack-picker-stack-column',
                    {
                      [`${prefixCls}__stack-column--hidden`]: !currentValue[index] || !currentValue[index][valueMember as 'value'],
                    },
                  );
                  ...
                  return (
                    ...
                    >
                      <div className={`${prefixCls}__stack-column-wrapper`} onClick={(e) => stopPropagation(e)}>
                        {
                          group.map((item, i) => {
                            const isActive = currentValue[index]
                              && currentValue[index][valueMember as 'value'] === item[valueMember as 'value'];

                            const columnItemClass = classNames(
                              `${prefixCls}__stack-column-item`,
                              {
