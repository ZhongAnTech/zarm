        ...
        <div className={`${prefixCls}-key`}>
          {
            this.getKeys().map((text, i) => {
              ...
              return <div className={keyCls} key={+i} onClick={() => this.onKeyClick(text)}>{(text === 'close') ? <Icon type="keyboard" /> : text}</div>;
            })
          }
        </div>
        <div className={`${prefixCls}-handle`}>
          <div className={`${prefixCls}-handle-item`} onClick={() => this.onKeyClick('delete')}><Icon type="deletekey" /></div>
          <div className={`${prefixCls}-handle-item ${prefixCls}-handle-item-ok`} onClick={() => this.onKeyClick('ok')}>确定</div>
        </div>
      </div>
    );
  }
