import { getTop, getLeft, getBoundingClientRect, setStyle, getStyleComputedProperty, isFixed } from '../dom';
  ...
  describe('dom', () => {
    describe('#getTop', () => {
      ...
      it('should get offset top recursively', () => {
        const testElement = ({ offsetTop: 100, offsetParent: { offsetTop: 50 } } as unknown) as HTMLElement;
        const actual = getTop(testElement);
    ...
    describe('#getLeft', () => {
      ...
      it('should get offset left recursively', () => {
        const testElement = ({ offsetLeft: 100, offsetParent: { offsetLeft: 50 } } as unknown) as HTMLElement;
        const actual = getLeft(testElement);
        expect(actual).toEqual(150);
      });
    });
    describe('#getBoundingClientRect', () => {
      ...
      it('should get bounding client rect for modern browser', () => {
        const testElement = ({
          getBoundingClientRect: jest.fn().mockReturnValueOnce({ top: 10, left: 9, right: 19, bottom: 20 }),
        } as unknown) as Element;
        const userAgentSpy = jest.spyOn(navigator, 'userAgent', 'get').mockReturnValueOnce('Mozilla/5.0');
